...
describe("base.css", () => {
  ...
  it("объявляет `color-scheme` для обоих режимов — режим приложения переведён браузеру", () => {
    // Единственная строка базового слоя, которая говорит браузеру о виде, и остаётся она не
    // по недосмотру (решение по `PWEB-39`): режим выбирает приложение и несёт классом `dark`
    // на корне, а здесь тот же выбор сказан на языке браузера. Скажи это кто-то ещё — у
    // одного ответа стало бы два источника; не скажи никто — от режима отстали бы системные
    // элементы: полосы прокрутки, галочки, автозаполнение.
