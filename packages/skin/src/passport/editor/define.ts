// Design notes: ./README.md#define

import type { ComponentPassport, PassportSettingName } from "../form/index.js";
...
// `Passport`, inferred from the `passport` ARGUMENT itself (`PWEB-207` follow-up) — not the widened
// `ComponentPassport<Part>` this took before. `StatesOf<Passport, P>`/`ValuesOf<Passport, S>`
// (`PassportEditorSpec`'s `parts`/`settings` fields, `./types.ts`) have nothing to read per-part
// states or per-setting values FROM otherwise; the widened interface never carried them past
// `definePassport`'s own `const` (`PWEB-207`) in the first place.
...
>(passport: Passport, spec: PassportEditorSpec<Part, Registry, Data, Passport>): PassportEditorInfo<Part, Registry, Data, Passport> {
  ...
  // Loose on purpose: `spec.settings`'s real type only has a key for each setting THIS passport
  // actually declares (`PWEB-207` follow-up), but `settingNames` is typed as the full closed
  // vocabulary (`PassportSettingName`) — runtime-narrower than the type here, not the other way
  // round, so indexing needs the wider shape even though every actual lookup below stays in bounds.
