...
describe("условия под отрицанием", () => {
  // Отрицание живёт в формуле, а не в условии: условие говорит «сумма больше ста», а нужно ли
  // обратное — решает логика сборки. Наружу это едет состоянием строки условия, и посчитать
  // его можно только здесь.
  it("прямая ссылка отрицанием не является", () => {
    expect([...negatedIds(parse("1 И 2"))]).toEqual([]);
  });

  it("под `НЕ` попадает только то, что под ним стоит", () => {
    expect([...negatedIds(parse("НЕ 1 И 2"))]).toEqual(["a"]);
  });

  it("отрицание группы накрывает всё внутри неё", () => {
    expect([...negatedIds(parse("НЕ (1 ИЛИ 2)"))].sort()).toEqual(["a", "b"]);
  });

  it("ДВОЙНОЕ отрицание отрицанием не является — считается чётность", () => {
    // `НЕ (НЕ 1)` это 1. Пометить такое условие отрицаемым значило бы соврать оформлению,
    // которое эту пометку покажет человеку.
    expect([...negatedIds(parse("НЕ (НЕ 1)"))]).toEqual([]);
  });

  it("тройное отрицание — снова отрицание", () => {
    expect([...negatedIds(parse("НЕ (НЕ (НЕ 1))"))]).toEqual(["a"]);
  });

  it("одно условие бывает и прямым, и отрицаемым — тогда оно отрицаемое", () => {
    // Оба вхождения законны: «1 И НЕ 1» — пустой отбор, но разобрать его надо, а не упасть.
    expect([...negatedIds(parse("1 И НЕ 1"))]).toEqual(["a"]);
  });
});
