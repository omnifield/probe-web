// Канал к агенту: «описал словами» → отбор. Контракт целиком — `PROBEWEB-9`.
//
...
// отвечает строгий вывод модели, за смысл `parseFilter` на клиенте (`PROBEWEB-8`).
...
export async function askForPreset({ text, fields, apiKey, fetchImpl = globalThis.fetch }) {
  ...
  try {
    ...
    try {
      ...
      // бы, что хранилище недоступно, хотя недоступна только модель (`PROBEWEB-8`).
      return {
        ok: false,
        status: 424,
