...
const FROZEN_ENTRYPOINTS = [
  ...
  `${BUILD}/vite`,
  RUNTIME,
  // Корневой вход стилевого слоя добавлен `tasker:PROBEWEB-53`: скелет обязан назвать палитру
  // и маркер базы ИМЕНАМИ, а не строками. Литерал в placed-once-файле — вторая копия чужого
  // имени, которая переживёт переименование и разъедется молча; починить её у уже созданных
  // продуктов нельзя ничем, кроме правки руками. Цена принята и названа: точек стало семь.
  STYLE,
  `${STYLE}/base.css`,
  `${STYLE}/themes.css`,
  UI,
 ...
 *
 * Имена из `${STYLE}` — не оформление, а ЗНАНИЕ ЧУЖОЙ ЗОНЫ, которое скелет обязан принимать:
 * идентификатор дефолтной палитры и имя токена-маркера базы. Их литералы в placed-once-файле
 * запрещены (`kb:PROBEWEB-13`, `kb:PROBEWEB-18`), и запрет держит `skin-place.test.ts`.
 */
const FROZEN_IMPORTS: Record<string, readonly string[]> = {
  [RUNTIME]: ["checkStyleOrder", "mount", "restoreSkin"],
  [STYLE]: ["BASE_MARKER", "DEFAULT_PALETTE"],
  [UI]: ["Button"],
...
describe("точка входа скелета", () => {
  ...
  it("монтирует приложение одним вызовом", () => {
    // Рядом с `mount()` из той же точки приезжает механика скина (`tasker:PROBEWEB-53`):
    // подпуть у рантайма один, отдельного пакета механика не заводит (`kb:PROBEWEB-13`).
    expect(main).toContain(
      `import { checkStyleOrder, mount, restoreSkin } from "${RUNTIME}";`,
    );
  ...
  it("подключает CSS стилевого слоя напрямую и побочкой", () => {
    ...
    // Ступени палитр — вторая строка, а не избыточность: базовый слой держит инвариант «ни
    // одного литерального цвета», значения приходят отсюда.
    //
    // Но одних двух импортов МАЛО, и с 2026-08-19 это надо говорить вслух: палитра цепляется
    // к документу только по имени, `:root` не красится ничем (`kb:PROBEWEB-18`). Файл
    // поставляется и подключается, а цвет даёт «названное место» в точке входа — его
    // стережёт `skin-place.test.ts`.
