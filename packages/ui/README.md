...
## Провайдер на каждый узел дерева — механизм уже есть, это `extras`

Компонент вроде `tree-view`, где часть внутри узла должна знать, В КАКОМ узле она находится
(`node`/`indexPath` у Ark's `TreeViewNodeProvider`), НЕ требует новой механики сборки. Три вещи,
которые уже есть и вместе решают задачу:

- **`defineKitComponent`'s третий аргумент** (`../../kit-form.js`) регистрирует компонент без
  адреса анатомии — `extras: { nodeProvider: TreeViewNodeProvider }` (`src/tree-view/components/
  kit.tsx`). Адресуется в сборке той же тильдой, что и везде: `tree-view.~nodeProvider`.
- **`AssemblyElement.props` — `Record<string, unknown>`, не JSON** (`packages/assembly/src/
  tree.ts`). Только `bind` ограничен JSON Pointer'ом; `props`, поставленный руками (не через
  декларативный `PassportAssembly`, а прямой сборкой `AssemblyTree`), может нести настоящий
  объект — коллекцию Zag, узел, массив `indexPath`.
- **`RenderNode` рекурсирует в детей `extra`-узла ТАК ЖЕ, как в детей обычной части** — никакого
  отдельного пути, ничего дописывать в `packages/assembly` не нужно.

**Обновление (2026-09-01): ловушки в форме тоже больше нет.** Раньше здесь было написано, что
ДЕКЛАРАТИВНЫЙ DSL (`repeat`/`bind`) не умеет посчитать нарастающий `indexPath` по мере
вложенности, и дереву нужна обычная функция вместо объявления через `PassportAssembly`. Заявка
закрыта — `PassportAssemblyElement`'s `indexPathBind?: string` (`packages/skin/src/passport/
assembly/nodes.ts`) называет проп, в который `baseAssemblyOf` сам кладёт накопленный путь литералом
(`packages/skin/src/passport/assembly/expand.ts`) — не как `bind`-путь, это структурный факт формы
дерева, известный уже на развороте, а не факт данных. Заодно нашёлся и починился соседний баг:
`ref` внутри `repeat` терял масштаб (пути внутри ссылки резолвились от корня данных, а не от места,
где ссылка реально стоит) — без него переиспользовать один шаблон ветки на нескольких уровнях
(группа → компонент → вариант) было бы нечем.

`tree-view`'s настоящие сборки (`playground/assemblies/base.ts`/`nested.ts`) теперь пишутся ТЕМ ЖЕ
языком, что и у аккордеона — `repeat`/`bind`/`indexPathBind`, без единой ручной функции. Живой
пример проверен тестом — `test/tree-view.test.tsx`. Обычная функция-строитель
(`test/node-provider.test.tsx`) осталась рядом как отдельное, более низкоуровневое доказательство
самого механизма `extras`, не единственный рабочий путь.
