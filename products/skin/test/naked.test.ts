// ГОЛЫЙ КИТ и границы витрины (`PWEB-31`, `PROBEWEB-11`).
//
...
describe("оболочка витрины несёт свой вид", () => {
  const css = readFileSync(resolve(process.cwd(), "src/showcase/showcase.css"), "utf8").replaceAll(
    /\/\*[\s\S]*?\*\//g,
    "",
  );

  it("значения свои — брать их больше не у кого", () => {
    // Фреймворк надеваемой палитры не везёт: палитра без рецептов это половина скина. Роли в
    // базовом слое объявлены через шкалы, а шкалы приходят СО СКИНОМ. Витрина же обязана быть
    // видна и тогда, когда скинов нет вовсе.
    expect(css).toContain("--ui-ink");
    expect(css).not.toMatch(/var\(--surface\)|var\(--text\)|var\(--brand-|var\(--space-/);
  });

  it("тёмная пара следует за классом режима", () => {
    // Класс ставит механика приложения и только при надетом скине: смотришь тёмный скин —
    // инструмент рядом тоже тёмный. Нет скина — нет класса, и оболочка светлая.
