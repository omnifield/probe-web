...
export function Knobs(props: { knobs: ReturnType<typeof createKnobs> }) {
  ...
  return (
    <div class="knobs">
      ...
      {/* ОТКУДА ПЕРЕЧЕНЬ — ГОВОРИТСЯ ВСЛУХ И ПРАВДУ, три состояния и ни одного молчаливого.
          Молчаливая работа означала бы, что человек сохранит пресет и не поймёт, почему его не
          видит коллега; НЕВЕРНАЯ подсказка хуже молчания — она отправляет искать не там.

          Прежняя редакция обещала «перечень встроенный», хотя запасного перечня в зоне нет с тех
          пор, как пресеты уехали в службу: человек видел пустой список и обещание списка. */}
      <Show
        when={k().source() === "служба"}
        fallback={
          <p class="knobs__refusal">
            {k().trouble() ?? "служба не отвечает"} — пресетов нет, а значит нет и оформления: кит
            остаётся голым. Сохранить пресет сейчас нельзя. Поднять службу:{" "}
            <code>pnpm --filter @probe-web/presets start</code>
          </p>
        }
      >
        {/* НЕТ ПРЕСЕТА — НЕТ СКИНА, и это рабочее состояние, а не поломка: удалить можно любой,
            включая семя. Кит остаётся голым — оформление необязательно (`kb:SKIN-7`, инвариант 4). */}
        <Show
          when={k().presets().length > 0}
          fallback={
            <p class="knobs__refusal">
              служба отвечает, но пресетов в ней нет — оформления тоже нет, кит голый. Вернуть
              семя: <code>pnpm run seed:presets</code>
            </p>
          }
        >
          <p class="knob__hint">перечень из службы — сохранённое здесь видят все</p>
        </Show>
      </Show>
      ...
      {/* ── ручки ──────────────────────────────────────────────────────────────────────── */}
      {/* ТУМБЛЕРА «Оформление» ЗДЕСЬ НЕТ (решение user 2026-08-18) — и это не потеря ручки, а
          снятая ложь. Оформление подключено ровно тогда, когда подключён пресет; отдельный
          тумблер позволял состояние «скин есть, пресета нет», в котором правила вида стоят, а
          значения берутся из умолчаний базы. Голый кит по-прежнему достижим (`kb:SKIN-7`,
          инвариант 4) — он там, где пресета нет. */}
      <div class="knob">
        <KnobLabel
          text="Режим"
