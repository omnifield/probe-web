...
describe("тёмная тема", () => {
  ...
  it("замес цвета — тоже по токенам: `color-mix` без токена внутри был бы тем же литералом", () => {
    ...
    // Сторожа «замесов должно быть больше нуля» здесь БОЛЬШЕ НЕТ, и это не послабление.
    // Он ставился против пустой проверки, пока замесы в стенде были; теперь весь вид
    // компонентов уехал в `@probe-web/skin`, и ноль замесов — законный ответ, а не признак
    // того, что проверка ослепла. Правило же остаётся: появится замес — он обязан брать
    // токен, иначе это тот же литеральный цвет, только в скобках.
