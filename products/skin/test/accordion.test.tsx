...
describe("содержимое и часть сосуществуют", () => {
  it("подпись стоит ПЕРЕД указателем — порядок выразим", () => {
    ...
    expect(первый?.textContent).toBe("Раздел");
...
describe("повтор части", () => {
  it("разделов несколько — иначе разделителей между ними не увидеть", () => {
    const host = draw();

    expect(host.querySelectorAll('[data-part="item"]').length).toBeGreaterThan(1);
  });

  it("нумеруется ТОЛЬКО различающая подпись", () => {
    const host = draw();
    ...
    // Разделы человек различает по названию — их номера уместны. Стрелка у всех одна: «стрелка 2»
    // соврала бы, что это другая вещь.
