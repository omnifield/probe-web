...
📂 Шестнадцать `read`, три `write` (`save_preset`, `report_feedback`, `save_content`) — итого девятнадцать,
...
| `tools/` | граница протокола — регистрация всех девятнадцати тулов через `registerTool`/`ok`/`err` | `registerTools(server)` |
...
|---|---|---|
| Все девятнадцать тулов регистрируются | `registerTools(server)` на реальном `McpServer`, `client.listTools()` | 19 тулов, annotations верные (16 `readOnlyHint`, 3 нет) |
