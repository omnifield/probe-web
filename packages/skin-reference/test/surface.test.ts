...
describe("эталон — чистые данные: в поставке ни одного импорта", () => {
  it("собранный файл не тянет за собой НИЧЕГО", () => {
    // Тип `Skin` приходит `import type` и стирается при эмите. Значит потребитель, взявший
  ...
  it("наружу едут запись и перечень одетых — и больше ничего", async () => {
    const поверхность = await import("../src/index.js");

    expect(Object.keys(поверхность).toSorted()).toEqual(["DRESSED", "referenceSkin"]);
  });
