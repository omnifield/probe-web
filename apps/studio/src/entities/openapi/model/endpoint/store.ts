...
export const endpointsAtom = persistAtom(createAtom<readonly Endpoint[]>([]), {
...
export const [currentEndpointId, setCurrentEndpointId] = createSignal<
  string | undefined
>();
...
export function removeEndpointsOfService(serviceId: string): void {
  for (const endpoint of endpointsAtom
    .get()
    .filter((one) => one.serviceId === serviceId))
    removeEndpoint(endpoint.id);
}

export function endpointsOfService(serviceId: string): readonly Endpoint[] {
  return endpointsAtom
    .get()
    .filter((endpoint) => endpoint.serviceId === serviceId);
}

export function updateEndpoint(
  id: string,
  patch: Partial<Omit<Endpoint, "id" | "serviceId">>,
): void {
  endpointsAtom.set((endpoints) =>
...
export function setEndpointHeaders(
  id: string,
  headers: readonly EndpointHeader[],
): void {
