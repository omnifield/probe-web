...
/** Тумблер трассы: `globalThis.__PROBE_WEB_ASSEMBLY_TRACE__ = true`. */
...
export function trace(label: string): () => void {
  ...
  return () => {
    ...
    console.debug(`[probe-web-assembly] ${label} — ${ms.toFixed(2)}ms`);
  };
}
...
export function note(message: string): void {
  if (!enabled()) return;
  console.debug(`[probe-web-assembly] ${message}`);
}
