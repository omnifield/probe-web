 ...
 *
 * У каждого есть КООРДИНАТА — вариация и состояние, — потому что случай без оси висит в потоке
 * при любом срезе и читается как «фильтр не работает». Человек называет не «что-то ещё», а тот
 * же адрес, только с условием, которого оси не знают.
 *
 * Чего здесь НЕ должно быть:
 *
 *  • **состояний, которые оси уже дают.** «Занята» — это состояние `busy`, оно на оси. Второй
 *    случай про то же дублировал бы ось и обещал бы, что это разные вещи;
 *  • **видов вместо намерений.** Вариация — намерение, а не «синяя» и не «плоская».
 *
 * Что здесь ОСТАЁТСЯ: условия, которых координатами не выразить, — длина содержимого, чужой
 * узел внутри, поведение кита вместо признака.
 */
export const HUMAN_CASES: Readonly<Record<string, readonly ShowcaseCase[]>> = {
  button: [
    human("button", {
      id: "disabled-real",
      title: "Отключена пропом",
      note: "не признаком, а пропом кита: проверяем, что он сам ставит `data-disabled`",
      at: { variant: "primary", state: "disabled" },
    ...
    human("button", {
      ...
      at: { variant: "primary", state: null },
      props: { children: "Сохранить и вернуться к перечню документов" },
    }),
    human("button", {
      id: "narrow",
      title: "В узком месте",
      note: "кнопка обязана уступать, а не выдавливать соседей: ширина ограничена нарочно",
      at: { variant: "primary", state: null },
      props: {
        children: "Сохранить и вернуться к перечню документов",
