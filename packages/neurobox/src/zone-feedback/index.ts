...
// (уже принята и реализована), ни поля сверх того, что там есть. Фидбэк ЗОНЫ — отдельная сущность
...
// не знает и не должен знать, что сегодня это тот же процесс, что у пресетов. Тулы конкретной зоны
// (apps/skin/.mcp/src/tools/feedback.ts) — отдельное ТЗ, не этот файл.
