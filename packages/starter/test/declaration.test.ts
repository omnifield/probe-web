 ...
 * (`PROBEWEB-1`, грабля первая). Образец в ТЗ показывал 7 — там же и назван их
 * дефектом. Проба стоит здесь, чтобы «подниму заодно» не уехало к потребителю отказом.
 */
const FORM_VERSION = 3;

/** Чужие артефакты: у файла один хозяин (`BASER3-2`), спор за них всплыл бы у потребителя. */
const NOT_OURS = [
  ".gitignore",
  ".devcontainer",
  "README.md",
...
describe("раскладка", () => {
  ...
  it("целится в web/, а не в корень", () => {
    ...
    // корень значит перемешанные скелеты (`PROBEWEB-4`).
  ...
  it("держит все пять классом placed-once", () => {
    ...
    // названа и принята: ошибку в скелете обвесом уже не починить (`BASER3-2`).
    for (const entry of declaration.layout) {
  ...
  it("рендерит только манифест, остальное везёт байт в байт", () => {
    ...
    // граблей, за которые baser уже заплатил (`PROBEWEB-1`).
...
describe("настройки", () => {
  it("объявлены ровно те, что действуют В МОМЕНТ УКЛАДКИ", () => {
    ...
    // застыл. То, что человек крутит потом, живёт за пакетами (`PROBEWEB-1`).
  ...
  it("даёт настройку версии каждому пакету, который называет манифест скелета", () => {
    ...
    // (`PROBEWEB-4`): дефолты проставляет architect перед публикацией.
  ...
  it("каждая несёт title, description, type и дефолт", () => {
    for (const [name, spec] of Object.entries(declaration.settings)) {
      ...
      // зная (`BASER3-34`). Настройка без дефолта означала бы вопрос пользователю, а
...
describe("состав поставки", () => {
  ...
  it("README едет вопреки files — значит его правка меняет поставку", () => {
    // npm всегда кладёт README, манифест и LICENSE (`PROBEWEB-1`). Проба стоит,
  ...
  it("публикуется в GitHub Packages", () => {
    // Пакет без метки `latest` дверь не видит вовсе (`BASER2-216`); переезд на
    // публичный npm объявлен и не начат (`ADR-19`).
