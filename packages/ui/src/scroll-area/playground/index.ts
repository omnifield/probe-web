...
//
// EDITOR-ONLY metadata for the scroll area. Human-facing text, taxonomy, and nesting rules for
...
// `settings.ts` — the scroll area has no setting from the closed vocabulary (`../entity/
// passport.ts`).
//
...
// dialog's own comment already names (`PWEB-34`) — a scroll area is not `disclosure` and no
// better-fitting section exists yet. `footprint` is real too (`footprintOf`, `PWEB-31`): the
// default `"regular"` — a scroll area's own real footprint is whatever the consumer puts inside
// it, not a fact this component can name for itself the way a table's own grid or a button's own
// single atom can.
...
export const editorInfo = /*@__PURE__*/ defineEditorInfo(passport, {
  ...
  footprint: "regular",
  variantAxis: {
    means: "the variant name a human gives the scroll area in the editor; the kit passes it through untouched",
  },
  parts,
  assemblies,
});
