...
// `defineEditorInfo` depends one-way on `passport` (the runtime contract in `accordion.passport.ts`,
// built on the bare parts in `accordion.anatomy.ts`), and nothing flows back, so a production
// bundle that never imports `/editor` never pays for a single word written below.
//
