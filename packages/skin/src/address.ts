...
/**
 * Тот же селектор, но БЕЗ ВЕСА — `:where()`.
 *
 * Нужен там, где условие в адрес поставила МЕХАНИКА, а не автор (`PWEB-103`): ось вариаций живёт
 * на корне, и часть, которую одевают, обращается к нему как к предку. Автор при этом предка не
 * писал — он написал «ввод при вариации X», — и вес его правила от нашего решения меняться не
 * вправе. Без `:where()` правило вариации на вложенной части внезапно перебивало бы правило
 * состояния той же части: вес вырос бы с трёх доводов до пяти, и порядок, которым распоряжается
 * порождение, перестал бы что-либо решать.
 *
 * Это тот же довод, по которому здесь всюду `:is()`: каскад не должен сдвигаться от того, КАК мы
 * записали условие. `:is()` берёт вес самого тяжёлого довода, `:where()` не берёт никакого —
 * разница ровно в том, чьё это условие.
 *
 * @param selector готовый селектор; пустой пробрасывается как есть — оборачивать нечего
 */
...
/**
 * Селектор ПРЕДКА вместе с его условиями — то, что встаёт слева от селектора части.
 *
 * Условия перечислены ОДНИМ списком, потому что вопрос у них один: чем узел предка себя
 * обозначил. Состояние приходит из словаря состояний части, вариация — из оси компонента
 * (`PWEB-103`), и складываются они одинаково: селектор части плюс признаки, объявленные
 * паспортом. Заведи вариация свой сбор селектора рядом — у одного вопроса стало бы два ответа, и
 * разошлись бы они молча.
 *
 * @param passport паспорт компонента-владельца
 * @param part часть-владелец
 * @param states состояния предка
 * @param variant селектор вариации, если условие адреса — она; пустая строка означает «вариация
 *   в адрес не входит»
