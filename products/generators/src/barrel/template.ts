...
/**
 * Turns a Handlebars template file into a `BarrelSpec.render` function: the
 * file is read and compiled once, and every call renders it against
 * `{ items }` — the array of ALL collected items, since a barrel is one
 * file built from every entry (`{{#each items}}`). This is what lets a
 * caller pass a template FILE instead of writing a render function by
 * hand — the template owns the text, `collect` owns the data.
 */
...
/**
 * The `ScaffoldSpec.render` counterpart of `fromTemplate`: a scaffold
 * renders ONE file per entry from ONE collected item, not an array of all
 * of them, so the item's own fields ARE the template's top-level context —
 * no `{ items }` wrapper, no `{{#each}}` needed to reach a single record's
 * fields.
 */
