...
// сборки — хотя `eslint.config.js`/`biome.json` потребителя импортируют собранный пресет.
//
 ...
 * `scripts` — сюда же: `build` материализует `dist/biome/biome.json` через
 * `scripts/generate-biome-json.mjs`, без него сборка падает на втором шаге.
 */
