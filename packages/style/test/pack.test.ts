...
describe("разрешение из установки", () => {
  ...
  it("подпутя `/values` НЕТ — дверь снята вместе с поводом её держать", () => {
    // Подпуть заводился как вход без реактивной части (`PWEB-44`); реактивной части не стало,
    // корень совпал с ним имя в имя (`PWEB-56`). Негатив обязателен: снятый ключ легко вернуть
    // «для совместимости», и вернулась бы вторая дверь к одному и тому же.
    //
    // Файл `dist/values.js` в поставке при этом ОСТАЁТСЯ и обязан остаться: он перечень
    // поверхности, и корень импортирует его. Снята дверь, а не файл — разница в `exports`.
    expect(() => req().resolve(`${PKG}/values`)).toThrow();
    expect(entries, "перечень поверхности пропал из поставки").toContain("dist/values.js");
  });
...
describe("Solid в чистой установке", () => {
  ...
  it("положительный контроль: СНЯТЫЙ подпуть в той же установке НЕ поднимается", () => {
    // Двойная работа, и обе нужны.
    //
    // Как контроль: без падающего импорта зелёные пробы выше значили бы и «Solid не нужен», и
    // «импорт вообще не выполнялся». Контролем взят именно снятый подпуть, а не выдуманное имя,
    // — так одна проба отвечает и «дверь закрыта», и «мы до двери дошли».
    expect(tryImport(`${PKG}/values`)).toMatch(
      /ERR_(PACKAGE_PATH_NOT_EXPORTED|MODULE_NOT_FOUND)/,
    );
  });
