...
не соврёт об успехе» (`NEUROBOX_CLIENT.md`).
...
**Реальный конверт бокса несёт `context: [{description, value}]` со `value`-строкой** (пример
`NEUROBOX_CLIENT.md` — `"value": "button"`). `unknown` пропускал мимо компилятора объект/число,
