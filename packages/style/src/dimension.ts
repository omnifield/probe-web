...
export function stepValue(scale: DerivedScale, step: DerivedStep): string {
  ...
  const product = parts.length === 1 ? seed : `calc(${parts.join(" * ")})`;

  // Сажаем на сетку СНАРУЖИ произведения, а не внутрь: округлять надо итог, а не сомножитель.
  // Округлённое семя дало бы одинаковые ступени (0.25rem × d при любом d из диапазона
  // садится обратно в 0.25rem) — то есть выключило бы плотность, а не сохранило сетку.
  return scale.snap ? `round(nearest, ${product}, ${GRID_STEP})` : product;
}
