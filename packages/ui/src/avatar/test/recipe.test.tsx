...
const outfit: Outfit = { name: "avatar-proof", palette: palette.name, forms: [form.name] };
const assembled = bound.assemble(outfit, { palettes: [palette], forms: [form] });

describe("avatar proof recipe — the passport dressed whole (PWEB-111)", () => {
  ...
  it("generates real CSS, not an empty rule set", () => {
    ...
    expect(css).toContain('[data-scope="avatar"]');
  });
