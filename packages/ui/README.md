...
Само поле `settings` в форме **необязательное**, и это названная уступка: форма общая на всех
поставщиков, и обязательное поле уложило бы разом каждый паспорт, объявленный до сегодня, — включая
синтетические паспорта соседних зон, которые к настройкам отношения не имеют. Обязательность
объявления **через `defineSettings`** держится в границах кита своей пробой
(`test/components.test.ts`): напиши поставщик `settings: {…}` мимо этой функции — обе половины
сверки пропали бы молча. Сделать поле обязательным на всех — ломающая правка общей формы, решение
architect.
