...
// `@pandacss/core` и postcss. Проверить его можно только тем, что уехало в `dist`: исходник об
// этом не говорит — важно, что попало в бандл после разрешения импортов.
//
...
describe("подпуть `./model` не тянет порождение", () => {
  it("в собранном `model.js` нет ни одной ссылки на взятую утилиту", () => {
    expect(bundle("model.js")).not.toContain("@pandacss/core");
  });

  it("и ни одной на postcss — он приезжает только вместе с ней", () => {
    expect(bundle("model.js")).not.toContain("postcss");
  });

  it("корневой вход её, наоборот, зовёт — иначе порождать было бы нечем", () => {
    expect(bundle("index.js")).toContain("@pandacss/core");
  });
});
...
describe("что уезжает, а что остаётся", () => {
  ...
  it("зависимость в поставке ровно одна — взятая утилита", () => {
    expect(Object.keys(manifest.dependencies)).toEqual(["@pandacss/core"]);
  });
