...
const FROZEN_ENTRYPOINTS = [
  ...
  `${BUILD}/vite`,
  RUNTIME,
  // Корневой вход стилевого слоя стоит здесь ради ОДНОГО имени — маркерного токена базы.
  // Литерал в placed-once-файле — вторая копия чужого имени, которая переживёт переименование
  // и разъедется молча; починить её у уже созданных продуктов нельзя ничем, кроме правки
  // руками. Цена принята и названа: точек шесть.
  STYLE,
  `${STYLE}/base.css`,
  UI,
];
 ...
 *
 * Из `${STYLE}` приезжает ровно одно имя — токен-маркер базы: ЗНАНИЕ ЧУЖОЙ ЗОНЫ, которое
 * скелет обязан принимать, а не повторять строкой. Второго имени тут больше нет: надеваемого
 * набора цветов по умолчанию поставка не везёт, и называть скелету нечего.
 */
const FROZEN_IMPORTS: Record<string, readonly string[]> = {
  [RUNTIME]: ["checkStyleOrder", "mount"],
  [STYLE]: ["BASE_MARKER"],
  [UI]: ["Button"],
...
describe("поверхность зон, вызываемая скелетом", () => {
  ...
  it("не помнит растворённый kit", () => {
    // Три точки 0.1.0 пересмотрены один раз и больше никогда (`PROBEWEB-4`). Ссылка,
...
describe("точка входа скелета", () => {
  ...
  it("монтирует приложение одним вызовом", () => {
    // Рядом с `mount()` из той же точки приезжает проверка порядка: подпуть у рантайма один,
    // отдельного пакета под неё не заводят.
    expect(main).toContain(`import { checkStyleOrder, mount } from "${RUNTIME}";`);
  ...
  it("не ищет точку монтирования сама", () => {
    ...
    // оказался бы заморожен заодно (`PROBEWEB-4`).
  ...
  it("подключает CSS стилевого слоя напрямую и побочкой", () => {
    ...
    // падает с TS2882 (дефект найден зоной `reference` до публикации).
    expect(main).toContain(`import "${STYLE}/base.css";`);
    // И это ЕДИНСТВЕННЫЙ CSS-подпуть скелета. Второго — с надеваемым набором цветов — у
    // поставки больше нет: цвет приходит скином, а скин у потребителя свой.
    const css = [...main.matchAll(new RegExp(`import "(${STYLE}/[^"]+)";`, "g"))];
