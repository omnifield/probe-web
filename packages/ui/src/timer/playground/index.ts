...
// dialog's own comment already names (`PWEB-34`) — a timer is not `disclosure` and no
// better-fitting section exists yet. `footprint` is real too (`footprintOf`, `PWEB-31`):
// `"compact"` — a handful of digits and a couple of buttons, the same small-atom bracket the
// icon/checkbox already sit in. `variantAxis.means` is prose — left as "TODO" for whoever fills
// the playground zone next, same as every state's `means` in `parts.ts`.
