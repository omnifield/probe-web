...
Составленный не несёт, и это не потеря: у составленного вида нет, он им и не был. Предел тот же,
что у цепочки зацепок: **чужая обёртка посередине** оставляет адрес внешнему — метка стоит
только на наших примитивах. Предел назван пробой, а не спрятан.
...
Компонент — это **папка** (`src/button/`: разметка, анатомия, пробы, `index.ts`), и в поставку
его паспорт попадает самим фактом объявления: вход подпути `./passport` порождает
`scripts/build.mjs` обходом папок. Файла, в который дописывает строку каждый новый компонент,
...
Паспортов пока два, и они выбраны не случайно. **Кнопка** — свой компонент, которого у Ark нет:
