...
describe("vite build конфигом из зоны `build`", () => {
  ...
  it("в бандл попали ОБА слоя стилей — базовый слой пакета и оформление приложения", () => {
    ...
    // переименуют токен там — проба поедет следом, а не начнёт врать (`PROBEWEB-13`).
    //
    ...
    // Носитель — поле, а не кнопка: кнопку с ступени 2 одевает РЕЦЕПТ скина, и её селектора
    // здесь больше нет вовсе (`test/skin.test.ts` стережёт, что двух путей к одному виду нет).
