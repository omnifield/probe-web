// СЕМЯ СЛУЖБЫ — первый скин кнопки, написанный человеком (`PWEB-31`).
//
// ## Это семя, а не запасной перечень
//
// Скины живут в СЛУЖБЕ (`store.ts`), и локального перечня у зоны нет: два перечня расходятся
// молча. Здесь остаётся ровно одно семя — то, чем засевают пустую службу командой
// (`pnpm run seed:skins`). Витрина его не читает и в ход не пускает: прочти она его при пустой
// службе, человек не отличил бы «служба отдала мой скин» от «показываю встроенный», а
