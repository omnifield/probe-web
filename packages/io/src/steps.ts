// ДЕЙСТВИЯ НАД ЗНАЧЕНИЕМ — перенос `products/tables/src/adapter/{model,steps}.ts` (PWEB-183):
// закрытый набор `Step`, разобранный по рынку ДО переноса (JSON-LD `@context`, R2RML, XSLT
// built-in template rules, Avro схема-эволюция — разбор целиком в `products/tables/src/adapter/
// model.ts`, не переписывается здесь заново). Снято ровно то, что сам файл называл временным:
// зависимость от `Row`/`FieldSpec` таблиц — обобщено до `Record<string, unknown>`.
//
// Каждое действие — чистая функция «значение → значение или причина, почему не вышло». Причина
// возвращается, а не бросается: не собравшееся поле — обычное состояние чужих данных, а не сбой
// программы.
