...
// он уходит в хранилище тем же куском JSON, каким пришёл (`PROBEWEB-8`). Отказ по существу
 ...
 * (`PROBEWEB-8`). Тридцать два символа — с запасом на любое осмысленное имя вида.
 */
...
/** Канал к агенту — вторая волна, контракт `PROBEWEB-9`. */
...
/**
 * Канал к агенту: описание словами → отбор. Контракт — `PROBEWEB-9`.
 *
...
async function askAgent(req, res, channel) {
  ...
  // читателя: ответ модели не привилегированнее файла переходника (`PROBEWEB-9`).
 ...
 * делает. Разные виды лежат в одном хранилище (`PROBEWEB-8`), и отдельных путей под них не
