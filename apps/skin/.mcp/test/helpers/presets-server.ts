...
function workspaceRoot(from: string): string {
  ...
  for (;;) {
    ...
    if (up === dir) throw new Error("pnpm-workspace.yaml not found above " + from);
    dir = up;
  }
}
...
async function waitForReady(url: string, timeoutMs: number): Promise<void> {
  ...
  throw new Error(`presets test server never became ready at ${url}: ${String(lastError)}`);
}
...
export async function spawnTestPresetsServer(): Promise<TestPresetsServer> {
  ...
  const child: ChildProcess = spawn("go", ["run", "./cmd/presets"], {
    cwd: backendDir,
    env: { ...process.env, PRESETS_PORT: String(port), PRESETS_HOST: "127.0.0.1", PRESETS_DB: dbPath },
  ...
  const exited = new Promise<void>((_res, rej) => {
    child.once("exit", (code) => {
      if (code !== null && code !== 0) rej(new Error(`presets test server exited early (${code}):\n${output}`));
    });
  });
  ...
  try {
    ...
    child.kill("SIGTERM");
    throw cause instanceof Error ? new Error(`${cause.message}\n\n${output}`) : cause;
  }
