// СПИСОК КАТАЛОГА — СТОР (`@omnifield/probe-web-store`), а не локальная константа страницы.
//
...
/** Стор каталога: разделы кита по группам. */
export const catalogStore = createStore({
  context: { groups: groupsOf() },
  on: {},
});

/** Разделы каталога — реактивный аксессор (`groups()`, не `groups`). */
export function useCatalogGroups() {
  return useSelector(catalogStore, (state) => state.context.groups);
}
