...
async function create(req, res, store, limits) {
  ...
  try {
    ...
    if (error instanceof StorageFull) {
      // 507 — про хранилище целиком, а не про этот запрос (RFC 4918 §11.5, сверено
      // 2026-08-13). Разница с 413 существенная для того, кто читает ответ: 413 чинится
      // уменьшением пресета, 507 — удалением чужого, и текст говорит ровно это.
      return fail(
        res,
        507,
        "storage_full",
