 ...
 *
 * Рамки в фикстуре нет НАМЕРЕННО, и это находка самого прохода. Добавь сюда
 * `borderColor: "var(--знак-8)"` — читаемость называет пару низкой: 1,68 при норме 3. Считает она
 * рамку против СОБСТВЕННОЙ заливки узла, а норма для внешней рамки читает её против того, что
 * СНАРУЖИ, — а этого механика не знает и знать не может.
 *
 * Обойти молча внутри пробы значило бы спрятать вопрос. Он поднят архитектору отдельно; здесь
 * фикстура просто не берёт на себя того, чего проход не проверяет.
 */
...
describe("рецепт → CSS", () => {
  ...
  it("читаемость на своём материале молчит", () => {
    expect(skinContrast(passageSkin, [PASSPORTS.button!])).toEqual([]);
  });
  ...
  it("и молчит не оттого, что считать нечем: испорченный материал она называет", () => {
    ...
    expect(skinContrast(spoiled, [PASSPORTS.button!]).length).toBeGreaterThan(0);
  });
