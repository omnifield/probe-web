...
// `fallback` (initials) and `image` (a real or broken `src` — worth showing BOTH: the pair's own
// opposite-polarity `visible`/`hidden` states, `../entity/passport.ts`'s own file header, only
// shows up with a real load or a real failure, not a static mock).
