...
function architectBanner(config) {
  return [
    ...
    ``,
    `- Триаж запросов user; решения, контракты и задачи овнерам — **в трекере, которым продукт пользуется сегодня**; спроси user, если не знаешь, какой это.`,
    ...
    `- Овнер до трекера может не дотянуться. Тогда ТЗ и отчёты ходят текстом через user, а в узел кладёшь их ты — не заставляй его ждать службу.`,
  ].join("\n");
}
...
function ownerBanner(config, { scope, paths, name }) {
  ...
  return [
    ...
    `- Перед первым Edit прочитай \`${firstPath}/README.md\` (если есть) и то, на что ссылается ТЗ.`,
    ...
    ``,
    `## Скоуп задачи`,
    `Работаешь по ТЗ — из трекера продукта либо текстом от user, — а НЕ по локальным файлам. Непонятен scope — STOP, спроси. Не угадывай.`,
    `Отчёт отдаёшь туда же, откуда пришло ТЗ. Не дотягиваешься до трекера — отдай текстом через user: архитектор положит в узел сам.`,
  ].join("\n");
}
