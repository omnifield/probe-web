...
остальные компоненты объявляются **по одному** волной разноса (`PWEB-7`). Компонент без
паспорта отдаёт `undefined` — честно, а не пустой заглушкой, которая выглядела бы как
