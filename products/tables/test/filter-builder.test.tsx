...
describe("сборка условий", () => {
  ...
  it("добавляет условие сравнения и нумерует его", () => {
    ...
    expect(one(host, "[data-slot='condition-number']").textContent).toBe("1");
  });
  ...
  it("набор операторов сужается типом поля", () => {
    ...
    const operators = () =>
      all<HTMLOptionElement>(host, "[data-slot='condition-operator'] option").map((node) => node.value);

    expect(operators()).toContain("contains");

    const field = one<HTMLSelectElement>(host, "[data-slot='condition-field']");
    field.value = "/amount";
...
describe("счётчик условия", () => {
  it("показывает «оставляет N из M» и отдельно неизвестные", () => {
    ...
    const count = one(host, "[data-slot='condition-count']");
    expect(count.textContent).toContain("оставляет 2 из 3");
    // Третья строка без поля — это НЕ «не подошла», и на экране это разные вещи.
    expect(one(host, "[data-slot='condition-unknown']").textContent).toBe(", неизвестно 1");
  });
  ...
  it("без неизвестных отдельной строки нет", () => {
    ...
    expect(one(host, "[data-slot='condition-count']").textContent).toContain("оставляет 2 из 3");
    expect(host.querySelector("[data-slot='condition-unknown']")).toBeNull();
  });
});
...
describe("своя логика", () => {
  ...
  it("включается флажком и заполняется формулой по умолчанию", () => {
    ...
    const toggle = one<HTMLInputElement>(host, "[data-slot='logic-toggle'] input");
    toggle.click();

    expect(state().logic.mode).toBe("formula");
    expect(one<HTMLInputElement>(host, "[data-slot='logic-input']").value).toBe("1 И 2");
  });

  it("флажок недоступен, пока условий нет", () => {
    const { host } = setup();
    expect(one<HTMLInputElement>(host, "[data-slot='logic-toggle'] input").disabled).toBe(true);
  });

  it("разобранная формула уезжает в состояние деревом по идентификаторам", () => {
    const { host, state } = setup(withTwo());
    one<HTMLInputElement>(host, "[data-slot='logic-toggle'] input").click();

    type(one<HTMLInputElement>(host, "[data-slot='logic-input']"), "1 ИЛИ 2");
  ...
  it("сломанная формула названа ТЕКСТОМ и помечена недействительной (WCAG 3.3.1)", () => {
    ...
    type(one<HTMLInputElement>(host, "[data-slot='logic-input']"), "(1 И 2");

    expect(one(host, "[data-slot='logic-error']").textContent).toContain("не хватает закрывающей скобки");
    expect(one<HTMLInputElement>(host, "[data-slot='logic-input']").getAttribute("aria-invalid")).toBe("true");
  ...
  it("известная поправка предложена кнопкой (WCAG 3.3.3)", () => {
    const { host, state } = setup(withTwo());
    one<HTMLInputElement>(host, "[data-slot='logic-toggle'] input").click();
    type(one<HTMLInputElement>(host, "[data-slot='logic-input']"), "1 И");

    const fix = one(host, "[data-slot='logic-fix']");
  ...
  it("условие, не упомянутое в формуле, названо — иначе оно молча не работает", () => {
    ...
    expect(one(host, "[data-slot='logic-unused']").textContent).toContain("2");
  });
  ...
  it("удаление условия, на которое ссылается формула, — видимое событие, а не сдвиг номеров", () => {
    ...
    press(all(host, "[data-slot='condition-remove']")[0]!);

    expect(state().conditions).toHaveLength(1);
    expect(one(host, "[data-slot='logic-error']").textContent).toContain("удалено");
    // Второе условие стало первым НА ЭКРАНЕ, но формула про это знает и показывает пропажу.
    expect(one<HTMLInputElement>(host, "[data-slot='logic-input']").value).toBe("? ИЛИ 1");
  });
});
