...
//
// Такой потребитель появился ровно один — и подпуть у зоны ровно один: `./passport`
// (`src/passport.ts`, `tasker:PWEB-2`). Паспорт читают механика скина и редактор, и им нужны
// ДАННЫЕ о частях, состояниях и вариантах, а не примитивы: через корневой вход перечень тянул
// бы за собой JSX, Solid и `@kobalte/core`. Отсюда же и обратное правило — паспорт из этого
// файла НЕ реэкспортируется: два входа к одним данным означали бы два обещания вместо одного.
//
