...
// ПЕРЕЧНЯ НЕТ — он был бы вторым источником правды, — поэтому список в этом случае пуст, а без
// пресета снимается и оформление: остаётся голый кит, и это рабочее состояние (`kb:SKIN-7`,
...
export interface Knobs {
  ...
  /**
   * Стоит ли оформление. ТОЛЬКО ЧТЕНИЕ: своей ручки у него нет — оно следует за пресетом.
   * Есть пресет — одето, нет пресета — голый кит. Третьего состояния не существует.
   */
  dressed: () => boolean;
  dark: () => boolean;
 ...
 *
 * СВОЕГО ТУМБЛЕРА У ОФОРМЛЕНИЯ НЕТ (решение user 2026-08-18): оформление подключено ровно тогда,
 * когда подключён ПРЕСЕТ. Прежняя редакция вешала тег при монтировании и снимала его отдельной
 * ручкой — и стенд без единого пресета показывал одетые компоненты: правила вида стояли, значения
 * приходили из умолчаний базы. Человек видел «скин есть, пресетов нет» и был прав, считая это
 * поломкой: оформление зоны — это пресет плюс правила, и половина без второй половины не состояние
 * продукта, а его отсутствие, показанное как присутствие.
 *
 * «Голый кит» (`kb:SKIN-7`, инвариант 4) от этого не пропадает, а становится ЧЕСТНЕЕ: он ровно
 * там, где пресета нет, — и добраться до него можно, удалив пресеты, а не отдельной ручкой стенда.
 */
function useSkinTag() {
...
export function createKnobs(api: PresetsApi = createPresetsApi()): Knobs {
  ...
  // пуст, об этом сказано человеку, и оформление снимается вместе с пресетом: остаётся голый
  // кит, оформление необязательно (`kb:SKIN-7`, инвариант 4).
   ...
   * тогда вид не подключается вовсе. Буквально: вместе с пресетом снимается и `skin.css`, поэтому
   * остаётся ГОЛЫЙ КИТ, а не одетые компоненты на умолчаниях базы — половина оформления читалась
   * бы как поломка (решение user 2026-08-18). Это рабочее состояние (`kb:SKIN-7`, инвариант 4);
   * возврат к семени — в одну команду: `pnpm run seed:presets`.
   */
  ...
  createEffect(() => {
    ...
    if (current === undefined) {
      // НЕТ ПРЕСЕТА — НЕТ СКИНА, и снимается всё сразу: правила вида, тема и плотность. Снимать
      // что-то одно значило бы показывать половину — либо атрибут, для которого значений никто не
      // объявлял, либо одетые компоненты без пресета. Оба варианта человек читает как поломку, и
      // читает верно.
      skin.apply(false);
