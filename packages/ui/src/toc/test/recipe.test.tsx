...
const outfit: Outfit = { name: "toc-proof", palette: palette.name, forms: [form.name] };
const assembled = bound.assemble(outfit, { palettes: [palette], forms: [form] });

describe("toc proof recipe — the passport dressed whole", () => {
  ...
  it("generates real CSS, not an empty rule set", () => {
    ...
    expect(css).toContain('[data-scope="toc"]');
  });
