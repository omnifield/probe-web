...
> версию значило бы молча потерять два узла, которые кит на самом деле адресует. Подробнее —
> `FAQ.md`.
...
> проверено по коннектору компонента-первоисточника. `root`, `control` и `indicatorGroup` своих
