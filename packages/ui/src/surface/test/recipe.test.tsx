...
const outfit: Outfit = { name: "surface-proof", palette: palette.name, forms: [form.name] };
const assembled = bound.assemble(outfit, { palettes: [palette], forms: [form] });

describe("surface proof recipe — the passport dressed whole (PWEB-111)", () => {
  ...
  it("generates real CSS, not an empty rule set", () => {
    ...
    expect(css).toContain('[data-scope="surface"]');
  });
