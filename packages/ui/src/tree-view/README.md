...
**Пять частей, не семнадцать.** `root` / `item` / `itemControl` / `controlIndicator` /
`itemContent` — это всё, что видит автор сборки. Ark различает лист и ветку СВОИМИ раздельными
наборами частей (`item`/`itemText`/`itemIndicator` против `branch`/`branchControl`/…/
`branchContent`) — в нашей анатомии этого различия нет: `item` один на оба случая.
...
**`itemControl`/`controlIndicator`/`itemContent` — наши, не Ark'овские.** Добавлены через
`entity/anatomy.ts`'s `extendWith(...)` (тот же построитель анатомии, каким Ark строит родные
части — адрес получается настоящим, `partSelector`/рецепты его видят как обычную часть, без
особых случаев). `itemControl` группирует подпись и индикатор (зеркалит `branchControl` для
ветки, но своего клика может и не решать, если рисуется как лист — там всё уже решает `item`).
`itemContent` — открытый слот, содержимое решает потребитель, своего вида не несёт.

**Как `itemControl`/`controlIndicator`/`itemContent` узнают, лист они сейчас рисуют или ветку.**
...
старой версии: `itemContent` рисуется то простым `<div>` (лист), то `ArkBranchContent` (ветка) —
...
объявляет `open`/`closed` у `itemContent` (`states: []`), так что явного стейта-исключения
завести нельзя — `display` там вообще не тронут, часть просто ничего не решает про раскладку
своего содержимого. Та же логика для `controlIndicator`: она одна на вращающуюся стрелку ветки
(всегда видна) и прячущуюся галочку листа (нативный `hidden`, пока не `selected`) — `display`
положен только внутри состояний `open`/`closed`/`selected`, не в базе.
