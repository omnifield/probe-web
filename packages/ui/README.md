...
### Базовая сборка: как выглядит рабочий экземпляр
...
Объявляется вложенным деревом **частей**, отдаётся плоской картой с корнем — той формой, в которой
дерево читает механика сборки:

```ts
const дерево = baseAssemblyOf(passportOf("accordion"));
// { components: { root: "accordion", nodes: { accordion: {…}, item: {…}, "item-2": {…}, … } } }

baseAssemblyOf(passportOf("accordion"), "ui.accordion"); // адрес — ВХОД, а не константа
```
...
**Сборка одна, и это решение:** перечень сборок превратил бы паспорт в витрину. Условия
(«наведённый», «раскрытый») ставятся осями поверх неё теми, кто их показывает.

**Форма отвергает сборку, не сходящуюся с паспортом** — на объявлении, а не значением в отчёте:
корень сборки бывает только корневой частью; часть мимо анатомии не адресуема ничем; каждый
вложенный узел проверяется **тем же `admits`**, которым его проверит редактор. Собери поставщик
экземпляр, которого редактор собрать не даст, — расхождение вскрылось бы у человека.
...
```ts
variables: [{ name: "--height", means: "измеренная высота раскрытого содержимого", setBy: "kit" }]
```
