...
export type WorkspaceProps<T extends ValidComponent = "div"> =
  PolymorphicProps<T> & {
    /**
 ...
 * ```
 */
export const Workspace = slotAware(function Workspace<
...
export type WorkspaceHeaderProps<T extends ValidComponent = "div"> =
  PolymorphicProps<T>;

/** Верхняя полоса — не на всю высоту, только над `main`/`rightbar`. */
export const WorkspaceHeader = slotAware(function WorkspaceHeader<
  T extends ValidComponent = "div",
...
export type WorkspaceSidebarProps<T extends ValidComponent = "div"> =
  PolymorphicProps<T>;

/** Левая колонка — во всю высоту, рядом с шапкой и показом сразу. */
export const WorkspaceSidebar = slotAware(function WorkspaceSidebar<
  T extends ValidComponent = "div",
...
export type WorkspaceMainProps<T extends ValidComponent = "div"> =
  PolymorphicProps<T>;

/** Показ — единственный слот, который есть всегда. */
export const WorkspaceMain = slotAware(function WorkspaceMain<
  T extends ValidComponent = "div",
...
export type WorkspaceRightbarProps<T extends ValidComponent = "div"> =
  PolymorphicProps<T>;

/** Правая колонка — необязательна; не положена в разметку, колонка схлопывается сама. */
export const WorkspaceRightbar = slotAware(function WorkspaceRightbar<
  T extends ValidComponent = "div",
...
export type WorkspaceFooterProps<T extends ValidComponent = "div"> =
  PolymorphicProps<T>;

/** Нижняя полоса — необязательна; не положена в разметку, строка схлопывается сама. */
export const WorkspaceFooter = slotAware(function WorkspaceFooter<
  T extends ValidComponent = "div",
