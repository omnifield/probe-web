...
Ловушка не в движке, а в форме: ДЕКЛАРАТИВНЫЙ DSL (`repeat`/`bind`, которым пишутся
`playground/assemblies/*` у всех остальных компонентов) не умеет посчитать НАРАСТАЮЩИЙ
`indexPath` по мере вложенности — `repeat` разворачивает один массив один раз, и всё. Рекурсивная
структура (дерево, вложенные группы) нуждается в обычной ФУНКЦИИ, которая обходит исходные данные
и строит плоский `AssemblyTree` напрямую, а не в объявлении через `PassportAssembly`. Живой,
проверенный тестом образец такой функции (`buildTree`, три уровня вложенности, настоящий
`indexPath`, настоящая глубина через `data-depth`) — `src/tree-view/test/node-provider.test.tsx`.
