...
> схема не может.
>
> `backdrop`/`positioner` портализуются в `document.body` сами, кит несёт это внутри себя —
> потребителю оборачивать их в `Portal` вручную не нужно (в отличие от примеров на ark-ui.com,
> где `Portal` — часть композиции, которую пишет каждый потребитель заново). `trigger` остаётся на
> месте в разметке — портал получают только те две части, чей DOM реально должен всплыть поверх
> страницы.
...
обычным CSS, не привязан к триггеру.
