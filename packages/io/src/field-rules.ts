// ПРАВИЛО ПОЛЯ + ОТЧЁТ — перенос `products/tables/src/adapter/{model,apply}.ts` (PWEB-183):
// цель конверсии обобщена до ОДНОЙ ЗАПИСИ (не «набор строк» — это допущение сам `model.ts`
// называл временным). Поиск «где в чужом ответе лежит массив записей» (`AdapterSpec.rows`,
// `discoverRowSets`) сюда НЕ переносится — это структурный вопрос (совпадает форма ЦЕЛИКОМ или
// нет), предмет L3 (`PWEB-184`), а не значений одной уже найденной записи.
...
/**
 * Что делать, когда поле не собралось. Рынок отвечает на этот вопрос тремя разными способами
 * без общей нормы (JSON-LD молча отбрасывает, Avro поднимает ошибку либо берёт умолчание,
 * protobuf терпит) — свой выбор не один на всё: у каждого поля он свой.
 */
...
/**
 * L2 как `codec` (PWEB-182 продолжение): `decode` — запись их формы → запись нашего канона по
 * правилам полей; бракованная (`reject`) запись — явный отказ `decode`, не тихий `null` (тот же
 * приём, что и `IoRegistry.require`).
 *
 * `encode` СЕГОДНЯ НЕ РЕАЛИЗОВАН (направление задаётся явно `"input"`, `PWEB-181`'s `IoDirection`)
 * — обратный путь для шагов вроде `dictionary`/`date`/`round` неоднозначен без дополнительного
 * решения о том, как каждый шаг инвертируется, и это решение этим тикетом не принимается.
 */
