...
устройстве, которое его обходит. Средство, а не решение — механика не приносит своего вида,
...
У движка нет DOM-анатомии — вместо частей компонента здесь возможности, каждая в своём файле
...
У движка нет одной сущности с переключателями, как у компонента, — есть один конструктор
...
| Настоящий `createContext`/`useContext` через `RenderNode`/`RenderTree` | owner-цепочка Solid не рвётся ни `<For>`, ни `<ErrorBoundary>`, ни `<Dynamic>`, ни самим `RenderTree` — на двух уровнях дерева | `test/context-repro.test.tsx` (PWEB-214) |
...
Хуки `RenderTree` независимы и комбинируются без спец-режима — каждый добавляется отдельным
пропом, ни один не требует другого. Это и есть рецепт редактора: та же одна функция, что рисует
голую витрину компонента без единого пропа, рисует и полноценный конструктор страниц — разница
только в том, сколько пропов ей дали.
