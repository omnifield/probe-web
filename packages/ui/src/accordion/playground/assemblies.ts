...
export const assemblies: readonly PassportAssembly<AccordionPart>[] = [
  ...
  },
  {
    name: "catalog",
    means:
      "как «filled», но у каждого раздела ВЛОЖЕННЫЙ повтор — кликабельный список, extra на " +
      "каждый пункт, а не один текст: два числа (сколько разделов, сколько пунктов в разделе) " +
      "не названы нигде, кроме самих данных (PWEB-156, вложенный повтор)",
    // Та же форма, что у "filled", до `itemContent` — там, вместо одного текстового узла, ВТОРОЙ
    // повтор по `components` (относительно текущего раздела — без ведущего `/`), один extra
    // `catalogItem` на элемент. Extra поставляет потребитель (настоящий кликабельный узел,
    // `PWEB-152`) — кит его по имени не знает, тот же довод, что у extras вообще
    // (`components/index.tsx`'s own header).
    tree: {
      part: "root",
      children: [
        {
          ...
          template: {
            ...
            children: [
              ...
              },
              {
                ...
                children: [
                  {
                    repeat: { path: "components" },
                    template: {
                      extra: "catalogItem",
                      bind: { label: "id", pressed: "pressed" },
                      on: { click: { event: { name: "select", context: { component: { path: "id" } } } } },
                    },
                  },
                ],
              },
