...
⚙️ Два раннера под одну идею («вход + шаблон → нужный нам выход»), для двух непохожих сред.
...
потребитель — `packages/ui`: `generators/generate.config.ts` → `kitBarrelPlugins` →
...
шаблон под конкретный диалект спеки превращает его в `Endpoint[]`/`Service`.
