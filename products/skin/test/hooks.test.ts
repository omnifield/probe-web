...
const ALLOWED_WITHOUT_SLOT = new Set([
  ...
  "to",
  "from",
  // Корень документа: место, где объявляются наши собственные переменные (набор значков).
  ":root",
]);

/**
 * `data-icon` — ВТОРАЯ зацепка зоны, и единственная СВОЯ.
 *
 * Всё остальное оформление цепляется за `data-slot` кита: имена придумал он, мы их читаем. Но
 * значок ставит ПОТРЕБИТЕЛЬ там, где ему нужно (`<span data-icon="check">`), — узла под это у
 * кита нет и быть не может, он про поведение. Поэтому имя атрибута объявляет зона, и оно такая
 * же публичная поверхность, как `data-variant`: уехало в чужую разметку — не меняется.
 *
 * Проба это ЗНАЕТ поимённо, а не «пропускает селекторы без зацепки»: третья зацепка не должна
 * появиться незаметно, потому что каждая — обещание, которое потом нельзя забрать.
 */
const OWN_HOOKS = ["[data-icon"];
  ...
  for (const file of skinFiles()) {
    const targets = () =>
      selectors(file.text).filter((s) => {
        ...
        // ПСЕВДОЭЛЕМЕНТ добавляет МЕТКУ, а не вид, и метка обязана пережить композицию: шеврон
        // открывашки говорит «это раскрывается» — при `as={Button}` он нужен ровно так же.
        // Вид самого узла эта поблажка не затрагивает: его правила стоят без псевдоэлемента и
        // проверяются наравне со всеми.
        if (head.includes("::after") || head.includes("::before")) return false;
  ...
  for (const file of skinFiles()) {
    it(`${file.name}: каждый селектор содержит [data-slot~=…]`, () => {
      const bad = selectors(file.text).filter(
        (s) =>
          !ALLOWED_WITHOUT_SLOT.has(s) &&
          !s.includes("[data-slot~=") &&
          !OWN_HOOKS.some((hook) => s.startsWith(hook)),
      );
