...
describe("base.css", () => {
  ...
  it("не одевает документ: кроме сброса и режима, своих объявлений в базовом слое нет", () => {
    ...
    expect([...new Set(declared)].sort()).toEqual(["box-sizing", "color-scheme", "margin"]);
  });

  it("`color-scheme` объявляет СПОСОБНОСТЬ к обоим режимам, а не один из них", () => {
    // Решение по `PWEB-50`. Прежде базовый слой называл конкретный режим — светлый на корне,
    // тёмный при классе, — и обосновано это было тем, что режим выбирает приложение, а мы лишь
    // переводим его выбор браузеру. Предпосылки больше нет: режим принадлежит СКИНУ, и назвать
    // его базе нечем — скина она не знает.
    //
    // Молчать при этом нельзя: отсутствие объявления браузер читает как выбор светлого.
    // Остаётся третье — «умею обе, следуй за человеком».
    expect(strip(built)).toContain("color-scheme: light dark");
  });

  it("конкретного режима базовый слой НЕ называет — ни на корне, ни по классу", () => {
    // Негатив держит гейт честным: без него зелёный прогон означал бы и «объявлена
    // способность», и «рядом остался прежний выбор». Класс режима принадлежит скину, и
    // базовый слой о нём не знает ВООБЩЕ.
    const code = strip(built);
    expect(code).not.toMatch(/color-scheme:\s*light\s*;/);
