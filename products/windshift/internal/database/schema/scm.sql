...
CREATE TABLE IF NOT EXISTS scm_providers (
	...
	github_org_id INTEGER,                        -- Stable org ID for GitHub App discovery (survives org renames)
	-- OAuth token storage (after OAuth flow completion)
...
CREATE TRIGGER IF NOT EXISTS cleanup_expired_scm_oauth_state
AFTER INSERT ON scm_oauth_state
BEGIN
...
CREATE TABLE IF NOT EXISTS item_scm_links (
	...
	smart_commits_applied_at DATETIME,            -- When smart-commit actions for a merged PR body were last applied (prevents re-runs)
	created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
	updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
...
-- Durable inbox/outbox for coding-agent PR review requests. The cursor above
-- is discovery-only; this ledger owns idempotency, retries and reply delivery.
...
-- Stable ownership makes review routing independent from whichever agent most
-- recently touched the linked item.
...
CREATE TABLE IF NOT EXISTS issue_sync_items (
	...
	github_issue_id INTEGER NOT NULL,                -- GitHub's internal issue ID (int64)
...
CREATE TABLE IF NOT EXISTS issue_sync_comments (
	...
	github_updated_at DATETIME,                      -- GitHub comment updated_at for change detection
	created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
	updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
