...
/**
 * Паспорт гармошки — живой, из кита, и берётся он ради ОДНОГО свойства: у содержимого раскрытость
 * объявлена вместе с оговоркой `absentWhen` — признак приезжает не всегда.
 *
 * Изобразить такое состояние в пробном поле было бы легче и хуже: граница «вид против движения»
 * (`PWEB-99`) стоит ради настоящего компонента, и проверять её надо на том объявлении, ради
 * которого она заведена. Рядом у того же компонента есть надёжная раскрытость ПУНКТА — то же имя,
 * другое объявление, — и это второй повод взять живой паспорт: разницу между ними проба видит
 * только здесь.
 */
export const accordionPassport = PASSPORTS.accordion!;
