...
// поставщика паспортов, а не особенность кита. Разбор критерия — в шапке `passport-form.ts`.
//
// ## Что здесь чинится
//
...
//
// Ровно по той же причине, по которой рядом с формой живёт `admits`: правило одно на всех
// читателей паспорта. Узкая запись формы на стороне механики порождения (`rules.ts`) была бы
// ВТОРОЙ копией — расходящейся молча.
//
...
// и читаются напрямую. Завести под них «типы для скина» и значило бы сделать ту самую вторую
// копию.
//
...
//
// `data-scope`/`data-part` даёт `@zag-js/anatomy`, а не мы: читать живой узел раньше, чем
// известен его паспорт, можно только по ИМЕНИ атрибута, которое знаешь заранее, — паспорт-то
// этот атрибут и опознаёт. Форма ЦЕЛИКОМ уже стоит на допущении «анатомия — от Zag» (см. шапку
// `passport-form.ts`), и это не новое допущение, а то же самое, применённое ещё раз.
//
// Кит держит те же два имени в `packages/ui/src/address.ts` — там ими пользуется СВОЙ, кит-
// specific механизм (`slot-chain.ts`, отбрасывающий чужой адрес перед спредом на компонент Ark).
// Общий дом для обеих копий был бы правильным ответом, но здесь его завести нельзя: `PWEB-110`
// запрещает трогать `packages/ui` даже на одну строку, а копия кита обслуживает механизм,
// который остаётся ТАМ. Второй копии в третьем месте это не отменяет — она унаследована, а не
// заведена заново.

import { PART, SCOPE, type PassportLookup } from "./address.js";
