...
/**
 * Убирает СВОЙ файл в сборке соседа — состояние свежего клона, где `build` ещё не запускали.
 *
 * Именно файл, а не папку `dist` целиком (`PWEB-100`). Раньше убиралась папка, и это стоило
 * мигающей пробы: раннер гоняет файлы ПАРАЛЛЕЛЬНО, а `generated-css.test.ts` держит в той же
 * папке свой `css/written.css` — и получал в ответ страницу вместо стилей, потому что файла на
 * диске уже не было.
 *
