...
//
// EDITOR-ONLY metadata for the file upload. Human-facing text, taxonomy, and nesting rules for
...
// `settings.ts` — the file upload has no setting from the closed vocabulary (`../entity/
// passport.ts`).
//
...
// as an input — `group: "inputs"`, the same rationale (a control the user provides a value with).
// `variantAxis.means` is prose — left as "TODO" for whoever fills the playground zone next, same
// as every state's `means` in `parts.ts`. `footprint` is real too (`footprintOf`, `PWEB-31`):
// `"regular"` — a dropzone plus a file list has real content, the same middle bracket the field's
// own recipe sits in, not a small atom and not inherently full-row.
...
export const editorInfo = /*@__PURE__*/ defineEditorInfo(passport, {
  ...
  footprint: "regular",
  variantAxis: {
    means: "the variant name a human gives the file upload in the editor; the kit passes it through untouched",
  },
  parts,
  assemblies,
});
