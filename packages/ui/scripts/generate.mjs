...
// Взято второе: сборка — это `vite build` (дважды) плюс отдельный `tsc` деклараций, а
// порождение — два обхода каталога и две записи. Гнать первое ради второго значит платить
// сборкой за каждую проверку типов. Шаг отдельный и в графе Nx (`package.json`, `nx.targets`).
//
