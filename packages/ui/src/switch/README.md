...
Переключатель «вкл/выкл» — тумблер-дорожка с ползунком, а не квадрат с галочкой: тот же смысл, что
у чекбокса (два состояния, участие в форме), но для настроек, которые действуют сразу по клику, а
не для выбора элемента из списка.
