...
/**
 * Страница стенда. Каждая — про ОДНУ вещь, чтобы не смешивать разговоры в одном экране.
 *
 * Страниц ровно две, и третьей — «таблица» — нет намеренно (решение user 2026-08-13):
 * управление колонками переехало В САМУ КОЛОНКУ, под её название, и отдельной странице про
 * устройство вида стало нечего показывать. Таблица есть на обеих страницах — как способ
 * посмотреть результат.
 */
export type PageId = "adapter" | "filters";
...
export const PAGES: readonly PageMeta[] = [
  {
    ...
    hash: "#/adapter",
    nav: "Переходник",
    title: "Переходник: чужая форма — в наш канон",
    lead:
      "Выбирается форма, в которой приходят данные, — к ней подставляется свой переходник. " +
      "Всё, что ниже, про эти различия уже не знает: один словарь полей на отбор, таблицу и график.",
  },
  {
    id: "filters",
    hash: "#/filters",
...
export const START: PageId = "adapter";
